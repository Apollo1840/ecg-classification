...
def train_model(model_svm_path, tr_features_scaled, tr_labels, C_value, gamma_value, multi_mode):
...
def eval_model(svm_model, features, labels, multi_mode, voting_strategy, output_path, C_value, gamma_value, DS):
    ...
    if multi_mode == 'ovo':
        ...
        prob_ovr = 0.5  # fake number, I do not understand here
...
def main(
    ...
    if do_cross_val:
        ...
        perf_measures_path = create_svm_model_name(
            '/home/mondejar/Dropbox/ECG/code/ecg_classification/python/results/' + multi_mode,
            winL, winR,
            do_preprocess,
            maxRR, use_RR, norm_RR,
            compute_morph,
        ...
        if do_cross_val == 'pat_cv':  # Cross validation with one fold per patient
            ...
            print("You must specify which kind of crossval type you use, eg 'pat_cv' or 'beat_cv'.")
    else:
        ...
        if os.path.isfile(model_svm_path):
            ...
            svm_model = train_model(model_svm_path, tr_features_scaled, tr_labels, C_value, gamma_value, multi_mode)
